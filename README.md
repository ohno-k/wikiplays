...
- **記事の知名度 5 段階**: 常識レベル 〜 超マニアック (または おまかせ) をプレイヤーが選択。Wikipedia の閲覧数 (直近 60 日の 1 日平均) を基準にした絶対評価で段階を決める (`FameScorer`)。常識レベルの記事はランダム取得ではほぼ当たらないので、ジャンルのカテゴリ内で被リンク数の多い記事を別途取得して補充する (`ArticlePoolService.fetchAndStoreFamous`)
